...
char *createdFileNames;
long createdFileNamesBufferLen;
long createdFileNamesCount;
char *createdFileTypes;
long createdFileTypesBufferLen;
long createdFileTypesCount;
char *createdHeaders;
long createdHeadersBufferLen;
long createdHeadersCount;
char currentHeader[256];

int warn(char *message) { MessageBox(NULL,message,"Warning",MB_ICONEXCLAMATION); return 1; }

#if 0
int munpack(char *fileName,
            char *createdFileNamesBuffer,long cbBuffer1,long *countFilesCreated,
            char *createdFileTypesBuffer,long cbBuffer2,long *countTypesCreated,
            char *createdHeadersBuffer,long cbBuffer3,long *countHeadersCreated) {

  createdFileNames = createdFileNamesBuffer;
  createdFileNamesBufferLen = 0;
  createdFileNamesCount = 0;
  createdFileTypes = createdFileTypesBuffer;
  createdFileTypesBufferLen = 0;
  createdFileTypesCount = 0;
  createdHeaders = createdHeadersBuffer;
  createdHeadersBufferLen = 0;
  createdHeadersCount = 0;

  memset(createdFileNames,0,cbBuffer1);
  memset(createdFileTypes,0,cbBuffer2);
  memset(createdHeaders,0,cbBuffer3);

  FILE *fTemp = fopen(fileName,"rt");

  handleMessage(part_init(fTemp),"text/plain",0,1);

  fclose(fTemp);

  *countFilesCreated = createdFileNamesCount;
  *countTypesCreated = createdFileTypesCount;
  *countHeadersCreated = createdHeadersCount;
...
int extractText;
{
   ...
   rc = handleMessage(inpart, "text/plain", 0, extractText);
   ...
   rc = handlePartial(inpart,headers,contentParams,extractText);
   ...
   rc = handleMultipart(inpart, contentType, contentParams, extractText);
   ...
   rc = handleUuencode(inpart, subject, extractText);
   ...
   rc = handleText(inpart, contentEncoding);
   ...
   rc = saveToFile(inpart, inAppleDouble, contentType, contentParams, contentEncoding, contentDisposition, contentMD5);
...
char *ParseHeaders(inpart, subjectp, contentTypep, contentEncodingp, contentDispositionp, contentMD5p)
...

{
    ...
    if (!alloced) {
   headers = malloc(alloced = HEADGROWSIZE);
    }
    next = headers;
...
char **headerp;
{
   ...
   if (nparam+1 >= palloced) {
       ...
       param = (char **) realloc((char *)param, palloced * sizeof(char *));
   }
   param[nparam++] = header;
...
char *key;
{
    ...
    if (!alloced) {
   value = malloc(alloced = VALUEGROWSIZE);
    }
