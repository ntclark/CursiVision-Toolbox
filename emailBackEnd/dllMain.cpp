...
#include "CursiVision_i.c"
   ...
   *ppObject = NULL;
   if ( CLSID_CursiVisionEmailBackEnd != rclsid  ) 
   ...
   OBJECT_NAME = "InnoVisioNate.CursiVisionEmailBackEnd";
   OBJECT_NAME_V = "InnoVisioNate.CursiVisionEmailBackEnd.1";
   OBJECT_VERSION = "1.0";
...
//sprintf(szTemp,"%ld",131473L);
   ...
   pICatRegister -> RegisterClassImplCategories(CLSID_CursiVisionEmailBackEnd,1,&categoryId);
   ...
   CATID categoryId = IID_ICursiVisionBackEnd;
   pICatRegister -> UnRegisterClassImplCategories(CLSID_CursiVisionEmailBackEnd,1,&categoryId);
   pICatRegister -> Release();
   ...
   *ppv = NULL; 
   EmailBackEnd *pef = new EmailBackEnd(punkOuter);
