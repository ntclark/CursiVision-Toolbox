...
#include "EnhancedReplicator.h"
    ...
    *pResult = S_FALSE;
    ...
    HRESULT theReplicator::_IGPropertyPageClient::TakePropertySheetDialogs(SAFEARRAY *pHwnds) { 

    UINT_PTR *ppData = NULL;

    SafeArrayAccessData(pHwnds,reinterpret_cast<void **>(&ppData));

    long lBound;
    long uBound;

    SafeArrayGetLBound(pHwnds,1,&lBound);
    SafeArrayGetUBound(pHwnds,1,&uBound);

    for ( long k = lBound; k <= uBound; k++ ) {
        HWND hwndPage = (HWND)(*ppData);
        PostMessage(hwndPage,WM_REFRESH_TEMPLATE_DOC,0L,0L);
        ppData++;
    }

    SafeArrayUnaccessData(pHwnds);
    ...
    *pCount = 1;
    ...
    pPropSheetPages[0].dwFlags = PSP_USEICONID | PSP_USETITLE | PSP_DLGINDIRECT;
    ...
    adjustPropertiesDialogSize((DLGTEMPLATEEX *)pPropSheetPages[0].pResource,64);
