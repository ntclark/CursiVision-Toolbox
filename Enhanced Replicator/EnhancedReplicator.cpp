...
#include "pdfEnabler_i.c"
    ...
    memset(hdcDrawRestore,0,sizeof(hdcDrawRestore));
    memset(restoreRect,0,sizeof(restoreRect));
    memset(pageScrollTop,0,sizeof(pageScrollTop));
...
//      pICursiVisionServices -> FreeSignatureGraphic(pSignatureGraphics[theIndex]);
    ...
    if ( ! ( NULL == hdcDrawRestore[theIndex] ) ) {
        hdcDrawRestore[theIndex] = NULL;
    }
    ...
    long theReplicator::addReplicant(long sourceIndex,long moveX,long moveY) {
...
//      pSignatureGraphics[targetIndex] = (signatureGraphic *)pICursiVisionServices -> GetSignatureGraphic(fromIndex);
    ...
    pSG -> documentRect = pWritingLocations[fromIndex] -> documentRect;
