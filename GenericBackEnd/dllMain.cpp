...
#include "CursiVision_i.c"
   ...
   *ppObject = NULL;
   if ( CLSID_CursiVisionGenericBackEnd != rclsid  ) 
   ...
   OBJECT_NAME = "InnoVisioNate.CursiVisionGenericBackEnd";
   OBJECT_NAME_V = "InnoVisioNate.CursiVisionGenericBackEnd.1";
   OBJECT_VERSION = "1.0";
   memcpy(&OBJECT_CLSID,&CLSID_CursiVisionGenericBackEnd,sizeof(GUID));
   memcpy(&OBJECT_LIBID,&LIBID_CursiVisionGenericBackEnd,sizeof(GUID));
   OBJECT_DESCRIPTION = "CursiVision Generic Back End";
...
//      RegSetValueEx(keyHandle,NULL,0,REG_SZ,(BYTE *)szTemp,(DWORD)strlen(szModuleName));
   ...
   pICatRegister -> RegisterClassImplCategories(CLSID_CursiVisionGenericBackEnd,1,&categoryId);
   ...
   CATID categoryId = IID_ICursiVisionBackEnd;
   pICatRegister -> UnRegisterClassImplCategories(CLSID_CursiVisionGenericBackEnd,1,&categoryId);
   pICatRegister -> Release();
   ...
   *ppv = NULL; 
   ...
   if ( IID_IPropertyPage == riid ) {
      if ( ! GenericBackEnd::CurrentObject() )
         return E_UNEXPECTED;
      if ( GenericBackEnd::CurrentPropertyPage() )
         GenericBackEnd::CurrentPropertyPage() -> Release();
      GenericBackEnd::CreatePropertyPage();
      return GenericBackEnd::CurrentPropertyPage() -> QueryInterface(riid,ppv);
   }
#endif

   GenericBackEnd *pef = new GenericBackEnd(punkOuter);
