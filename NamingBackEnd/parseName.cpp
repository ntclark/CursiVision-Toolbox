...
long NamingBackEnd::parseName(char *pszPDFFile,char *pszName) {
   ...
   BSTR bstrInternal = SysAllocString(_wtempnam(NULL,L"cvtmp"));
   ...
   while ( p = strstr(pStart,"TJ") ) {
      ...
      if ( ! ( p = strstr(pResult,szNamePrefix) ) ) 
         continue;

      p += strlen(szNamePrefix);
