...
#include "CursiVision_i.c"
   ...
   *ppObject = NULL;
   if ( CLSID_CursiVisionNamingBackEnd != rclsid  ) 
   ...
   OBJECT_NAME = "InnoVisioNate.CursiVisionNamingBackEnd";
   OBJECT_NAME_V = "InnoVisioNate.CursiVisionNamingBackEnd.1";
   OBJECT_VERSION = "1.0";
...
//      RegSetValueEx(keyHandle,NULL,0,REG_SZ,(BYTE *)szTemp,strlen(szModuleName));
   ...
   CATID categoryId = IID_ICursiVisionBackEnd;
   pICatRegister -> UnRegisterClassImplCategories(CLSID_CursiVisionNamingBackEnd,1,&categoryId);
   pICatRegister -> Release();
   ...
   *ppv = NULL; 
   ...
   if ( IID_IPropertyPage == riid ) {
      if ( ! NamingBackEnd::CurrentObject() )
         return E_UNEXPECTED;
      if ( NamingBackEnd::CurrentPropertyPage() )
         NamingBackEnd::CurrentPropertyPage() -> Release();
      NamingBackEnd::CreatePropertyPage();
      return NamingBackEnd::CurrentPropertyPage() -> QueryInterface(riid,ppv);
   }
#endif
