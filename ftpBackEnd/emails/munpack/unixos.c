...
extern char *createdFileNames;
extern long createdFileNamesBufferLen;
extern long createdFileNamesCount;

extern char *createdFileTypes;
extern long createdFileTypesBufferLen;
extern long createdFileTypesCount;

extern char *createdHeaders;
extern long createdHeadersBufferLen;
extern long createdHeadersCount;
...
//       (void) mkdir(fname, 0777);
   ...
   descfname = malloc(strlen(fname)+6);
    }
    else {
   descfname = buf;
    }
 ...
 */
void os_closetypedfile(outfile)
FILE *outfile;
{
    fclose(outfile);
}
...
void os_warnMD5mismatch()
{
    ...
    warning = malloc(strlen(output_fname) + 100);
    sprintf(warning, "%s was corrupted in transit",output_fname);
    MessageBox(NULL,"Warning",warning,MB_ICONEXCLAMATION);//    warn(warning);
    free(warning);
}
