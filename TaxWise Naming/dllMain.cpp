...
#include "CursiVision_i.c"
   ...
   *ppObject = NULL;
   if ( CLSID_TaxWiseNamingBackEnd != rclsid  ) 
   ...
   OBJECT_NAME = "InnoVisioNate.TaxWiseNamingBackEnd";
   OBJECT_NAME_V = "InnoVisioNate.TaxWiseNamingBackEnd.1";
   OBJECT_VERSION = "1.0";
   memcpy(&OBJECT_CLSID,&CLSID_TaxWiseNamingBackEnd,sizeof(GUID));
   memcpy(&OBJECT_LIBID,&LIBID_TaxWiseNamingBackEnd,sizeof(GUID));
   OBJECT_DESCRIPTION = "TaxWise File Naming Tool";
...
//      RegSetValueEx(keyHandle,NULL,0,REG_SZ,(BYTE *)szTemp,(DWORD)strlen(szModuleName));
   ...
   pICatRegister -> RegisterClassImplCategories(CLSID_TaxWiseNamingBackEnd,1,&categoryId);
   ...
   CATID categoryId = IID_ICursiVisionBackEnd;
   pICatRegister -> UnRegisterClassImplCategories(CLSID_TaxWiseNamingBackEnd,1,&categoryId);
   pICatRegister -> Release();
   ...
   *ppv = NULL; 
   ...
   NamingBackEnd *pef = new NamingBackEnd(punkOuter);
