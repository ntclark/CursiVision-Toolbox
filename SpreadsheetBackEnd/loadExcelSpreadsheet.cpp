...
#include <time.h>
   ...
   if ( ! pIWorksheet ) {
      ...
      if ( IDNO == MessageBox(NULL,szMessage,"Error!",MB_YESNO | MB_DEFBUTTON1) ) {
      ...
      IDispatch *pIDispatch = pIWorksheets -> Add();
   ...
   for ( long k = 0; k < FIELD_COUNT; k++ ) {
      ...
      if ( ! szColumnId[k][0] ) {
         ...
         if ( includeDate ) {
            ...
            if ( ! szColumnId[k][0] ) {
               MessageBox(NULL,"You cannot include the date signed field without also specifying the column letter in the spreadsheet.","Error",MB_OK);
            } else {
         ...
         if ( includeTime ) {
            ...
            if ( ! szColumnId[k][0] ) {
               MessageBox(NULL,"You cannot include the time signed field without also specifying the column letter in the spreadsheet.","Error",MB_OK);
            } else {
         ...
         if ( includeDocumentLink ) {
            if ( ! szDocumentLinkColumn[0] ) {
               MessageBox(NULL,"You cannot include the document link without also specifying the column letter in the spreadsheet.","Error",MB_OK);
            } else {
               isHyperLink[k] = 1L;
