...
#include "EnhancedSpreadsheetBackEnd_i.c"
#include "CursiVision_i.c"
   ...
   *ppObject = NULL;
   if ( CLSID_CursiVisionSpreadsheetBackEnd != rclsid  ) 
   ...
   OBJECT_NAME = "InnoVisioNate.EnhancedCursiVisionSpreadsheetBackEnd";
   OBJECT_NAME_V = "InnoVisioNate.EnhancedCursiVisionSpreadsheetBackEnd.1";
   OBJECT_VERSION = "1.0";
   memcpy(&OBJECT_CLSID,&CLSID_CursiVisionSpreadsheetBackEnd,sizeof(GUID));
   memcpy(&OBJECT_LIBID,&LIBID_CursiVisionSpreadsheetBackEnd,sizeof(GUID));
   OBJECT_DESCRIPTION = "CursiVision Spreadsheet Tool";
...
//      RegSetValueEx(keyHandle,NULL,0,REG_SZ,(BYTE *)szTemp,(DWORD)strlen(szModuleName));
   ...
   pICatRegister -> RegisterClassImplCategories(CLSID_CursiVisionSpreadsheetBackEnd,1,&categoryId);
   ...
   CATID categoryId = IID_ICursiVisionBackEnd;
   pICatRegister -> UnRegisterClassImplCategories(CLSID_CursiVisionSpreadsheetBackEnd,1,&categoryId);
   pICatRegister -> Release();
   ...
   *ppv = NULL; 
   ...
   if ( IID_IPropertyPage == riid ) {
      if ( ! SpreadsheetBackEnd::CurrentObject() )
         return E_UNEXPECTED;
      if ( SpreadsheetBackEnd::CurrentPropertyPage() )
         SpreadsheetBackEnd::CurrentPropertyPage() -> Release();
      SpreadsheetBackEnd::CreatePropertyPage();
      return SpreadsheetBackEnd::CurrentPropertyPage() -> QueryInterface(riid,ppv);
   }
#endif

   SpreadsheetBackEnd *pef = new SpreadsheetBackEnd(punkOuter);
